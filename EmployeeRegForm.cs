    ...
    public partial class EmployeeRegForm : Form
    {
        ...
        private void UniqueNumberGenerator() // generates serial number for the database and Athlete ID
        {
            try
            {
                ...
                cmbEmployeeID.Items.Add(serialNo.ToString());
                cmbEmployeeID.Text = serialNo.ToString();
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e) // Update Button
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    query = "UPDATE Employees SET EmployeeName = '" + txtEmployeeName.Text + "', NIC = '" + txtNIC.Text + "', Gender= '" + gen +
                            "', DateOfBirth='" + dtpDOB.Text + "', ContactNo = '" + txtContactNo.Text + "', Address='" + txtAddress.Text +
                            "' WHERE EmployeeID= '" + cmbEmployeeID.SelectedItem.ToString() + "'";
                    con.Open();
                    cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
                    con.Close();

                    MessageBox.Show("Employee ID: " + cmbEmployeeID.SelectedItem.ToString() + " updated successfully");
                    Clear();
                }
                catch (Exception err)
                {
        ...
        private void cmbAthleteID_SelectedIndexChanged(object sender, EventArgs e) // Event handler for when an Athlete ID is selected in the ComboBox
        {
            try
            {
                if (cmbEmployeeID.SelectedIndex > 0)
                {
                    employeeId = cmbEmployeeID.SelectedItem.ToString();
                    ...
                    while (r.Read())
                    {
                        txtEmployeeName.Text = r.GetValue(1).ToString();
        ...
        private void btnDelete_Click_1(object sender, EventArgs e) // Delete button
        {
            ...
            {
                employeeId = cmbEmployeeID.SelectedItem.ToString();
                DialogResult res = MessageBox.Show("Are you sure you want to DELETE record " + employeeId, "Confirm to delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void btnSave_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    query = "INSERT INTO Employees(EmployeeID, EmployeeName, NIC, Gender, DateOfBirth, ContactNo, Address) VALUES('" +
                             cmbEmployeeID.Text + "','" + txtEmployeeName.Text + "','" + txtNIC.Text + "','" + gen + "','" + dtpDOB.Text + "','" +
                    ...
                    MessageBox.Show("Employee ID: " + serialNo + " successfully SAVED to the database!", "SAVE", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Clear();
                }
                catch (Exception err)
                {
        ...
        private void btnSearch_Click(object sender, EventArgs e) // Search button
        {
            try
            {
                ...
                if (tab.Rows.Count > 0)
                {
                    ...
                    cmbEmployeeID.Items.Clear();
                    cmbEmployeeID.Items.Add("--SELECT--");
                    foreach (DataRow line in tab.Rows)
                    {
                        cmbEmployeeID.Items.Add(line["EmployeeID"]);
                    }
                    cmbEmployeeID.SelectedIndex = 0;
                }
                else
